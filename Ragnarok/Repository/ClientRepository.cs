...
namespace Ragnarok.Repository
{
    public class ClientRepository : IClientRepository
    {
        ...
        public ICollection<ClientJuridical> FindByCnpj(string cnpj, int businessId)
        {
            try
            {
                return _context.ClientJuridical.Where(x => x.RegisterEmployee.BusinessId == businessId && x.CNPJ == cnpj).ToList();
            }
            catch (Exception e)
            {
        ...
        public ICollection<ClientPhysical> FindByCpf(string cpf, int businessId)
        {
            try
            {
                return _context.ClientPhysical.Where(x => x.RegisterEmployee.BusinessId == businessId && x.CPF == cpf).ToList();
            }
            catch (Exception e)
            {
        ...
        public ICollection<Client> FindByEmail(string email, int businessId)
        {
            try
            {
                return _context.Client.Where(x => x.RegisterEmployee.BusinessId == businessId && x.Email == email).ToList();
            }
            catch (Exception e)
            {
